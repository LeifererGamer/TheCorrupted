...
namespace TheCorrupted.src.Core.Models.Cards.Common
{
    internal class CleansingRitual() : CardModel(0, CardType.Skill, CardRarity.Common, TargetType.Self), ICustomModel
    {
        ...
        protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
        {
            ...
            {
                var amount = base.DynamicVars["Cleansing"].BaseValue;
                if (card.Owner.Creature.HasPower<DoomPower>())
                {
                    if (card.Owner.Creature.GetPower<DoomPower>().Amount <= base.DynamicVars["Cleansing"].BaseValue)
        ...
        {
            base.DynamicVars["Cleansing"].UpgradeValueBy(2m);
        }
