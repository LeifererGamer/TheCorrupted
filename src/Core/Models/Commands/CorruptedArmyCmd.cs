...
using TheCorrupted.src.Core.Models.Monsters;

namespace TheCorrupted.src.Core.Models.Commands
{
    public static class CorruptedArmyCmd
    {
        public static async Task<SummonResult> Summon(PlayerChoiceContext choiceContext, Player summoner, decimal amount, AbstractModel? source)
        {
            ...
            Creature army = combatState.Allies.FirstOrDefault((Creature c) => c.Monster is Osty && c.PetOwner == summoner2);
            ...
            else
            {
                ...
                {
                    army = await PlayerCmd.AddPet<Osty>(summoner2);
